...
const AddIncomeExpense = () => {
  ...
  const listDate =
    new Date(Date.now()).toLocaleString("en-US", { month: "short" }) +
    " " +
    new Date().getDate() +
    ", " +
  ...
  const expenseToDispatch = () => {
    ...
    dispatch(
      addListExpense({ text: type, money: amount, currentTime: listDate })
    );
  };
  ...
  const incomeToDispatch = () => {
    ...
    dispatch(
      addListIncome({ text: type1, money: amount, currentTime: listDate })
    );
  };
